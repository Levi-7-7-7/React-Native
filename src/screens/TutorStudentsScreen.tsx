...
const clearPdfCache = async () => {
  ...
  for (const dirPath of pathsToClean) {
    try {
      ...
      console.log(`Cache cleanup failed for directory: ${dirPath}`, e);
    }
  }
};
...
// ── UPGRADED LEDGER-STYLE PDF HTML GENERATOR ─────────────────────────────────
...
function buildPdfHtml(
  ...
  // Generate individual student table body structures
  const tableBodiesHtml = students
    .map((student, idx) => {
      ...
      if (approved.length === 0) {
        ...
        approved.forEach(cert => {
          ...
          rowGroupHtml += `
            <tr class="cert-item-row">
              <td class="cert-name-cell">${certName}</td>
              <td class="points-cell">${cert.pointsAwarded ?? 0}</td>
            </tr>
          `;
        });
      }

      // Encapsulate each student in an anti-split tbody element
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header { 
    ...
    width: 100%; 
    padding-bottom: 12px; 
    border-bottom: 2px solid #0f2864; 
    margin-bottom: 10px; 
  }
  .header-logo-cell {
    display: table-cell;
  ...
  /* Main Institutional Ledger Architecture */
  .ledger-table {
    width: 100%;
  ...
  /* Student Ledger Container blocks */
  .student-ledger-group {
    page-break-inside: avoid;
  }

  /* Primary Identity Data Rows */
  .student-profile-row {
  ...
  /* Nested Detail Certificate Rows */
  .cert-item-row td {
  ...
  .cert-name-cell {
    ...
    font-size: 9.5px;
    font-weight: 500;
    padding-left: 20px !important; /* Visual inset alignment */
  }
  .empty-certs-text {
    color: #94a3b8;
    font-style: italic;
  ...
  /* Analytical Metric Balance Columns */
  .points-cell {
    text-align: center;
    font-weight: 600;
    color: #475569;
...
.material-icons {
  ...
  font-weight: normal;
  font-style: normal;
  font-size: 36px;       /* ← increase this to make it bigger */
  line-height: 1;
...
<div class="page">
  ...
  <!-- Unified Accounting Table Framework -->
  ...
  <div class="report-meta-footer">
    Generated on <b>${today}</b> &nbsp;|&nbsp; Comprehensive Registry Size: <b>${students.length} Record${students.length !== 1 ? 's' : ''}</b>
  </div>
</div>
</body>
</html>`;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (generatedFilePath) {
        try {
          const exists = await RNFS.exists(generatedFilePath);
          if (exists) {
            await RNFS.unlink(generatedFilePath);
          }
        } catch (cleanupError) {
          console.log('Failed to delete source cache file:', cleanupError);
        }
      }
      await clearPdfCache();
      setPdfLoading(false);
      ...
      {pdfLoading && (
        <View style={[styles.pdfBanner, {backgroundColor: colors.primaryMuted, borderColor: colors.primary}]}>
          ...
          <Text style={[styles.pdfBannerText, {color: colors.primary}]}>
            Generating Ledger PDF Report… compilation ongoing
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  trophyAppBadge: {  
    flexDirection: 'row',      // ← add this if not present
    alignItems: 'center',      // ← and this
    gap: 3,                    // ← tight spacing between icon and text
    paddingHorizontal: 6,
    paddingVertical: 2,
    borderRadius: 6,
